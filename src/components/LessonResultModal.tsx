...
interface LessonResultModalProps {
  ...
  onRetry: () => void;
  onReview: () => void;
  lesson?: { ui_mode?: string; title?: string };
}
...
const LessonResultModal: React.FC<LessonResultModalProps> = ({
  ...
  const isWarmUp = lesson?.ui_mode === "familiarization";
  return (
        ...
        <div className="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-950/95 px-6 py-5 text-slate-100 shadow-2xl">
          <div className="mt-6 text-center">
            ...
            <p className="mt-1 text-slate-400 text-sm">
              You’ve finished this warm-up. Move on to practice to lock in your
              skills.
            </p>

            {/* decorative circle */}
            <div className="mt-4 flex justify-center">
              <div className="h-20 w-20 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-slate-900 font-bold text-xl shadow-xl">
                ✓
              </div>
            </div>
            ...
            <button
              onClick={onAdvance}
              className="mt-6 w-full rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-emerald-400"
            >
